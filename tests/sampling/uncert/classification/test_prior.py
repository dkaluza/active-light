...
from al.sampling.uncert import entropy_info, off_centered_entropy
...
def test_off_centered_entropy_is_equal_to_entropy_for_balanced_data(probas):
    ...
    assert torch.allclose(entropy_info(state), off_centered_entropy(state))
...
def test_off_centered_entropy_is_equal_to_entropy_for_uniform_max_location(probas):
    ...
    assert torch.allclose(
        entropy_info(state), off_centered_entropy_with_uniform_max(state)
    )
